...
export function computeNetworkStatus(nodes, faultedIds) {
  ...
  return nodes.map((n) => {
    ...
    // Sync panel: affected when any synchronized generator is faulted or loses power
    if (n.type === 'sync_panel' && (n.synchronizes || []).length > 0) {
      const syncAffected = (n.synchronizes || []).some(
        (genId) => faultSet.has(genId) || hasFaultedAncestor(genId)
      );
      if (syncAffected) return { ...n, status: 'affected' };
    }
